...
@Stateless
public class ReportMaxBuyClient {
    private static final Logger LOGGER = LoggerFactory.getLogger(ReportMaxBuyClient.class);


    public List<AssetsSumDTO> returnMostOftenPurchasedAssets(){

        Response response = ClientBuilder.newClient()
                .target("http://localhost:8082/api/report/retrieve/"
                        + ReportName.MOST_PURCHASED_ASSETS).request().get();
